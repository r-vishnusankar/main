  ...
  return (
    <div className="w-full min-w-0">
      ...
      <section className="px-8 py-14">
        ...
        {[
          { id: "create" as const, slot: "homepage-banner", label: "Homepage Banner", desc: "Wide hero banners that drive traffic.", ratio: "16:9", aspectStyle: { aspectRatio: "16/9" }, overlayTitle: "New Season Collection", overlaySub: "Shop the latest looks" },
          { id: "create" as const, slot: "product-card",    label: "Product Card",    desc: "Highlight products with clean cards.",  ratio: "3:4",  aspectStyle: { aspectRatio: "3/4"  }, overlayTitle: "Premium Quality",       overlaySub: "₹2,499 · Free shipping" },
          { id: "create" as const, slot: "instagram-post",  label: "Instagram Post",  desc: "Eye-catching square posts for feed.",  ratio: "1:1",  aspectStyle: { aspectRatio: "1/1"  }, overlayTitle: "New Drop ✦",             overlaySub: "Shop via link in bio" },
          { id: "create" as const, slot: "sale-promo",      label: "Sale Promo",      desc: "Bold banners built to drive urgency.", ratio: "16:9", aspectStyle: { aspectRatio: "16/9" }, overlayTitle: "Flash Sale — 70% Off",  overlaySub: "24 hours only" },
          { id: "create" as const, slot: "email-header",    label: "Email Header",    desc: "Branded headers for campaigns.",       ratio: "3:1",  aspectStyle: { aspectRatio: "3/1"  }, overlayTitle: "Exclusive Member Offer", overlaySub: "For subscribers only" },
          { id: "create" as const, slot: "festival",        label: "Festival / Event", desc: "Themed seasonal creatives.",          ratio: "1:1",  aspectStyle: { aspectRatio: "1/1"  }, overlayTitle: "Diwali Sale 🪔",         overlaySub: "Celebrate with savings" },
          { id: "templates" as const, slot: "product-banner", label: "Product Banner", desc: "Combine product image with AI backgrounds.", ratio: "16:9", aspectStyle: { aspectRatio: "16/9" }, overlayTitle: "Crafted to Perfection", overlaySub: "Premium collection" },
          { id: "content-publish" as const, slot: "social-caption", label: "Social Caption & Blog", desc: "AI drafts captions, alt-text & blog copy.", ratio: "", aspectStyle: { aspectRatio: "1/1" }, overlayTitle: "AI-written caption ready", overlaySub: "Publish in one click" },
        ].map((item) => (
          <button key={item.slot} type="button" onClick={() => onNavigate?.(item.id)}
      ...
      <section className="px-8 py-16">
        ...
        {[
          { num: "01", title: "Upload your product & choose a purpose", desc: "Drop in your product image and pick what you're creating — homepage banner, product card, Instagram post, or sale promo. The right aspect ratio and a pre-filled prompt are set automatically.", cta: "Go to Create", nav: "create" as const, visual: <StepUploadMockup />, accentColor: "text-[var(--accent)]", borderColor: "border-[var(--accent)]/30", bgColor: "bg-[var(--accent)]/5" },
          { num: "02", title: "Pick a template & edit the prompt", desc: "Choose from professional e-commerce templates or your own custom ones. The prompt is pre-filled based on your purpose — edit it to match your brand voice, campaign, or seasonal event.", cta: "Browse Templates", nav: "templates" as const, visual: <StepPromptMockup />, accentColor: "text-purple-400", borderColor: "border-purple-500/30", bgColor: "bg-purple-500/5" },
          { num: "03", title: "Generate, edit & publish to platforms", desc: "AI generates your banner instantly. Review it in the editor, make tweaks, then download or publish directly to Facebook, Instagram, or WhatsApp — with optional IST scheduling.", cta: "Go to Publish", nav: "content-publish" as const, visual: <StepOutputMockup />, accentColor: "text-emerald-400", borderColor: "border-emerald-500/30", bgColor: "bg-emerald-500/5" },
        ].map((step) => (
          <div key={step.num} className="rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
            ...
            <div className="p-5">
              ...
              <button type="button" onClick={() => onNavigate?.(step.nav)} className={`inline-flex items-center gap-1.5 text-[13px] font-semibold ${step.accentColor} hover:underline`}>
                {step.cta} <IconArrow />
              </button>
            </div>
          </div>
          ...
          <div className="flex-shrink-0 w-full lg:w-[40%]">
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                t.custom ? (
                  <button key="custom" type="button" onClick={() => onNavigate?.("templates")}
